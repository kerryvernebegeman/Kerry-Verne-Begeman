...
def generate_from_generator_adaptive(args, Gs):
    psi = args.psi # 0.7
    radius_large = args.radius_large # 600.0
    radius_small = args.radius_small # 40.0
    current_position_increment = args.step1 # 0.005
    current_position_style_increment = args.step2 # 0.0025
...
def main(args):
    ...
    tflib.init_tf()
    _G, _D, Gs = pickle.load(open(pkl, "rb"))

    frames = generate_from_generator_adaptive(args, Gs)
